...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  useEffect(() => {
    const handleResize = () => {
      if (wrapperRef.current && textDimensions.width && textDimensions.height) {
        ...
        // При вычислении ограничений НЕ берём размер родителя через getBoundingClientRect —
        // он уже трансформирован scale из ComponentWrapper и даёт искажённые значения.
        // Используем offsetWidth/offsetHeight, которые возвращают layout-размер до transform.
        const parentEl = wrapperRef.current.parentElement;
        const viewportMargin = isMobile ? 0.95 : 0.9;
        const maxWidth = parentEl
          ? parentEl.offsetWidth * viewportMargin
          : window.innerWidth * viewportMargin;
        const maxHeight = parentEl
          ? parentEl.offsetHeight * viewportMargin
  ...
  return (
    <div
      ref={wrapperRef}
      // overflow-hidden убран — при scale > 1 он обрезал контент компонента
      ...
      style={{
        ...
        minHeight: '80px',
        maxWidth: '100%',
        // visible вместо hidden чтобы частицы и текст не обрезались при любом scale
        overflow: 'visible',
