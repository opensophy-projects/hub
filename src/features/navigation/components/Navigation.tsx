        ...
        <div style={{ flexShrink: 0, padding: mobile ? '10px 14px' : '8px 10px', borderBottom: `1px solid ${t.border}`, position: 'relative' }} ref={sectionRef}>
          <button
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', overflow: 'hidden', minWidth: 0, flex: 1 }}>
              ...
              {/* FIX: text cannot overflow and shift siblings */}
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.3, minWidth: 0 }}>{activeSection.navTitle}</span>
            </div>
...
const Navigation: React.FC<NavigationProps> = ({ currentDocSlug, toc = [], activeHeadingId = '' }) => {
  ...
  // Render nothing until we know the viewport — prevents any flash
  // useIsDesktopNav now initializes synchronously from window.innerWidth
