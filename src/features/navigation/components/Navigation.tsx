/**
 * Navigation.tsx — Rail + Panel navigation
 *
 * Desktop (>1000px):
 *   Rail (64px) слева + выдвижная панель
 *   Порядок кнопок: Панель → Тема → Поиск → Разделы → Оглавление → Наверх → Контакты
 *   Лого — не кнопка, просто картинка
 *
 * Mobile (≤1000px):
 *   Нижний бар с горизонтальным скроллом
 *   Порядок: {Панель} {Тема} Поиск Разделы [Лого] Оглавление Наверх {Контакты}
 *   {} — за краями (fade), видны при скролле влево/вправо
 */
...
function tk(isDark: boolean) {
  return {
    ...
    fgSub:      isDark ? 'rgba(255,255,255,0.2)'  : 'rgba(0,0,0,0.2)',
    ...
    mobBg:      isDark ? 'rgba(10,10,10,0.96)'    : 'rgba(220,219,215,0.96)',
...
function buildTree(docs: Doc[], query: string, navSlug: string): NavNode {
  ...
  const filtered = docs.filter(d =>
    (!q || d.title.toLowerCase().includes(q)) && (d.navSlug ?? '') === navSlug
  );
...
const DocLink: React.FC<{ doc: Doc; isDark: boolean; isActive: boolean; onClick?: () => void }> = memo(({ doc, isDark, isActive, onClick }) => {
  ...
  return (
    <a href={`/${doc.slug}`} onClick={onClick} style={{
      ...
      {doc.icon && (
        <span style={{ flexShrink: 0, width: 13, height: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', color: t.fgMuted }}>
          <LucideIcon name={doc.icon} size={12} />
        </span>
      )}
  ...
  return (
    <div>
      <button onClick={() => onToggle(path)} style={{
        ...
        {total > 0 && (
          <span style={{ fontSize: '0.67rem', padding: '1px 6px', borderRadius: '4px', background: t.accentSoft, color: t.fgMuted }}>{total}</span>
        )}
      </button>
      {expanded && (
...
const NavPanelContent: React.FC<{
  isDark: boolean; currentDocSlug?: string; onDocClick?: () => void; onOpenSearch: () => void;
            ...
            <div style={{
              ...
              {sections.map(s => (
                <button key={s.navSlug} onClick={() => {
                  ...
                  padding: '0.55rem 0.75rem', fontSize: '0.875rem', border: 'none',
...
// ─── TocPanelContent — с glow эффектом ───────────────────────────────────────
...
const TocPanelContent: React.FC<{
  ...
  if (!toc.length) return (
    <div style={{ padding: '2rem', textAlign: 'center', fontSize: '0.85rem', color: t.fgMuted }}>
    ...
    <nav style={{ padding: '6px 4px' }}>
      {toc.map((item, index) => {
        ...
        const shadow = isActive
          ...
          : glowOp > 0
            ? `inset 3px 0 8px -3px ${isDark ? `rgba(255,255,255,${glowOp * 0.35})` : `rgba(0,0,0,${glowOp * 0.35})`}`
            : 'none';
        return (
          <button
            key={item.id}
...
const PanelHeader: React.FC<{
  title: string; isDark: boolean; onClose: () => void; rightSlot?: React.ReactNode;
}> = ({ title, isDark, onClose, rightSlot }) => {
  const t = tk(isDark);
  return (
...
// ─── RailBtn — только иконка + текст, без фоновой подложки ───────────────────
  ...
  return (
    <button
      ...
      style={{
        ...
        color: isActive ? t.accent : hovered ? t.fg : t.fgMuted,
  ...
  return (
    <button
      ...
      style={{
        ...
        gap: '2px', padding: '0 12px', minWidth: '52px', border: 'none', background: 'transparent',
        cursor: 'pointer', color: isActive ? t.accent : t.fgMuted,
        fontSize: '9px', fontWeight: isActive ? 600 : 400, flexShrink: 0,
...
const DesktopNav: React.FC<{
  ...
  const [handleHovered, setHandleHov]   = useState(false);
    ...
    <>
      ...
      {railVisible && (
        <aside style={{
          ...
          {/* Лого — просто картинка, без ссылки */}
      ...
      {railVisible && activePanel && (
        <aside style={{
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            {activePanel === 'nav' && (
              <NavPanelContent isDark={isDark} currentDocSlug={currentDocSlug} onOpenSearch={() => setSearchOpen(true)} />
            )}
            {activePanel === 'toc' && (
          ...
          >
            {/* Видимая полоска */}
            <div style={{
              width: '3px', height: '56px', borderRadius: '3px',
              background: handleHovered ? t.handleHov : t.handle,
              pointerEvents: 'none',
            }} />
          </div>
        </aside>
...
const MobileNav: React.FC<{
  ...
  const fadeGrad = (dir: 'left' | 'right') =>
    isDark
      ? `linear-gradient(to ${dir}, rgba(10,10,10,0.92), transparent)`
      : `linear-gradient(to ${dir}, rgba(220,219,215,0.92), transparent)`;

  return (
    <>
      {/* Backdrop */}
      {sheet && (
        <div onClick={close} style={{ position: 'fixed', inset: 0, background: isDark ? 'rgba(0,0,0,0.5)' : 'rgba(0,0,0,0.2)', zIndex: 58, backdropFilter: 'blur(4px)' }} />
      )}

      {/* Шторки */}
      {(['nav', 'toc', 'contacts'] as const).map(s => (
        <div key={s} style={{
          position: 'fixed', bottom: '3.5rem', left: 0, right: 0, zIndex: 59,
          height: sheet === s ? SHEET_H[s] : 0,
          overflow: 'hidden', transition: 'height 0.28s cubic-bezier(0.4,0,0.2,1)',
        }}>
          <div style={{
            height: '100%', background: t.sheetBg,
      ...
      <nav style={{
        ...
        height: '3.5rem', background: t.mobBg,
        borderTop: `1px solid ${t.border}`,
        backdropFilter: 'blur(12px)',
        overflow: 'hidden',
      }}>
        {/* Fade-оверлеи — скрывают крайние кнопки */}
        <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 44, background: fadeGrad('right'), zIndex: 2, pointerEvents: 'none' }} />
        ...
        <div style={{
          ...
          {/* {Тема} — полускрыта */}
          <MobBtn label="Тема"     icon={isDark ? <Sun size={19} /> : <Moon size={19} />}          isDark={isDark} onClick={toggleTheme}        isActive={false} />

          {/* Видимая зона */}
          <MobBtn label="Поиск"    icon={<Search size={19} />}                                     isDark={isDark} onClick={() => { close(); setSearchOpen(true); }} isActive={false} />
          <MobBtn label="Разделы"  icon={<FolderOpen size={19} />}                                 isDark={isDark} onClick={() => toggle('nav')} isActive={sheet === 'nav'} />
          ...
          <MobBtn label="Оглавл."  icon={<List size={19} />}                                       isDark={isDark} onClick={() => toggle('toc')}      isActive={sheet === 'toc'} />
          <MobBtn label="Наверх"   icon={<ArrowUp size={19} />}                                    isDark={isDark} onClick={() => { close(); window.scrollTo({ top: 0, behavior: 'smooth' }); }} isActive={false} />

          {/* {Контакты} — скрыта справа */}
          <MobBtn label="Контакты" icon={<Mail size={19} />}                                       isDark={isDark} onClick={() => toggle('contacts')} isActive={sheet === 'contacts'} />
        </div>
      </nav>
