      ...
      {open && (
        <div className={`absolute left-3 right-3 mt-1 rounded-xl border shadow-xl z-50 overflow-hidden ${isDark ? 'bg-[#0f0f0f] border-white/10' : 'bg-[#E8E7E3] border-black/10'}`}>
          {sections.map(section => (
            <button key={section.navSlug} onClick={() => { onSelect(section.navSlug); setOpen(false); }} className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-sm transition-colors ${section.navSlug === activeSlug ? (isDark ? 'bg-white/10 text-white font-medium' : 'bg-black/10 text-black font-medium') : (isDark ? 'hover:bg-white/5 text-white/80' : 'hover:bg-black/5 text-black/80')}`}>
...
const SidebarHeader: React.FC<{ onClose: () => void; isDark: boolean; onToggleTheme: () => void; onToggleContacts: () => void; isDesktop: boolean }> = memo(({ onClose, isDark, onToggleTheme, onToggleContacts, isDesktop }) => (
  <div className="flex-shrink-0 px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)', backgroundColor: isDark ? 'rgba(10,10,10,0.97)' : 'rgba(232,231,227,0.97)', backdropFilter: 'blur(10px)' }}>
  ...
  <div className="flex-shrink-0 p-3">
    <div className="flex gap-2">
      <div className="relative flex-1">
        ...
        <input type="text" placeholder="Поиск по названию..." value={value} onChange={e => onChange(e.target.value)} className={`w-full pl-8 pr-3 py-2 rounded-lg text-sm border transition-colors outline-none ${isDark ? 'bg-white/5 border-white/10 text-white placeholder-white/40 focus:border-white/20' : 'bg-black/5 border-black/10 text-black placeholder-black/40 focus:border-black/20'}`} />
      </div>
  ...
  return (
    <div className={`fixed left-0 top-0 w-full md:w-80 h-screen border-r flex flex-col z-[60] ${isDark ? 'bg-[#0a0a0a] border-white/10' : 'bg-[#E8E7E3] border-black/10'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        {CONTACTS.map(c => (
          <a key={c.href} href={c.href} target={c.external ? '_blank' : undefined} rel={c.external ? 'noopener noreferrer' : undefined} className={`block px-3 py-2 rounded-lg text-sm transition-colors ${isDark ? 'text-white/70 hover:bg-white/5 hover:text-white' : 'text-black/70 hover:bg-black/5 hover:text-black'}`}>
  ...
  return (
    <>
      ...
      <div className="fixed left-0 top-0 h-screen z-50" style={{ width: isDesktop ? '20rem' : '100%' }}>
        {/*
          КАРТОЧКА: на десктопе — с padding 12px со всех сторон кроме правой (там прилегает к контенту),
          с border-radius 16px и GlowingBorder по всему периметру — точно как image 1.
        */}
        <div style={{ padding: isDesktop ? '12px 0 12px 12px' : '0', height: '100%', boxSizing: 'border-box' }}>
          <div
            className={isDark ? 'bg-[#0a0a0a]' : 'bg-[#E8E7E3]'}
            style={{
              position: 'relative',
              height: '100%',
          ...
          >
            {/* GlowingBorder — точная реализация GlowingEffect.tsx */}
            {isDesktop && (
              <GlowingBorder
                spread={25}
                movementDuration={2}
