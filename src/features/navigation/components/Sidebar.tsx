...
const iconBtn = (isDark: boolean) =>
  `p-3 rounded-lg transition-colors ${isDark ? 'text-white hover:bg-white/10' : 'text-black hover:bg-black/10'}`;
  ...
  >
    ...
    <div className="flex items-center gap-2">
      ...
      <button onClick={onClose} className={iconBtn(isDark)} aria-label="Закрыть меню">
        {isDesktop ? <PanelLeftClose size={24} /> : <X size={24} />}
      </button>
    </div>
  </div>
);
  ...
  <div className="flex-shrink-0 p-3 border-b" style={borderStyle(isDark)}>
    <div className="relative">
      ...
      <input
        ...
        className={`w-full pl-11 pr-3 py-2 rounded-lg text-sm border transition-colors outline-none ${
          isDark
            ? 'bg-white/5 border-white/10 text-white placeholder-white/50 focus:bg-white/10 focus:border-white/20'
            : 'bg-[#ddd8cd] border-black/10 text-black placeholder-black/50 focus:border-black/20'
        }`}
      />
    </div>
  </div>
    ...
    <a
      ...
      className={`block px-3 py-2 rounded-lg text-sm transition-colors ${
        isDark
          ...
          : 'text-black/80 hover:text-black hover:bg-black/8'
      }`}
    >
      {doc.title}
    </a>
  ...
  <div>
    <button
      ...
      className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-semibold transition-colors ${
        isDark ? 'text-white/90 hover:bg-white/8' : 'text-black/90 hover:bg-black/8'
      }`}
    >
      <div className="flex items-center gap-2">
      ...
      <span
        className={`text-xs px-2 py-0.5 rounded ${
          isDark ? 'bg-white/15 text-white/80' : 'bg-black/15 text-black/80'
        }`}
      >
        {docs.length}
      </span>
  ...
  <a
    ...
    className={`block px-3 py-2 rounded-lg text-sm transition-colors ${
      isDark
        ...
        : 'text-black/80 hover:bg-black/8 hover:text-black'
    }`}
  >
