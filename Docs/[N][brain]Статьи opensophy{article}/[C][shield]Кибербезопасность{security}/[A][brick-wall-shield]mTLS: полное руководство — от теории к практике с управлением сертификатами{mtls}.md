...
Поскольку Istio заточен в основном под k8s, а другие решения для меня показались довольно «большими» — вплане того чтобы выпускать сертификаты приходиться писать много команд, поэтому представляю вам [скрипт который упрощает выпуск сертификатов и делает управление намного легче](https://github.com/opensophy-projects/mtls). Скрипт заточен в основном под docker/podman итп контейнеры где стоит **Traefik**.
...
> Пресеты к путям также есть для Dokploy т.к это основная платформа для управления контейнерами и сайтами в opensophy проектах. [Вы можете создать запрос в гитхабе](https://github.com/opensophy-projects/mtls) чтобы добавить новые пресеты или сделать самому их.
