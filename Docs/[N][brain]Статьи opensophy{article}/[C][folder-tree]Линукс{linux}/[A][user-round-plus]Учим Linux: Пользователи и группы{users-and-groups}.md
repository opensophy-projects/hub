...
| Supplementary | `/etc/group`                     | Дополнительные права. Неограниченно (практически < 16). |
...
> ⚠️ На Ubuntu по умолчанию у root нет пароля. Используй `sudo -i` или `sudo su -` вместо `su`.
