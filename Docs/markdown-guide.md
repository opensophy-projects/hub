...
==Подчеркнутый текст== использует двойные знаки равенства.
...
Hub поддерживает математические формулы через KaTeX.
...
Квадратный корень: $\sqrt{x^2 + y^2}$
...
Hub поддерживает диаграммы Mermaid для визуализации процессов и структур.
...
Для отображения специальных символов используйте обратную косую черту:
...
> **Важно:** Используйте `async/await` для *асинхронных* операций.
>
> ```javascript
