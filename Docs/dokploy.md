...
**1 вариант: Установка через скрипт**

curl -sSL https://dokploy.com/install.sh | sh



![dokploy1.png](dokploy1.png "Частая ошибка со скриптом")

![dokploy2.png](dokploy2.png "Правильная установка если вы обычный пользователь вы сначала должны перейти в режим root, к примеру с помощью команды: sudo -i")
...
1. **Определение версии для установки**
...
7. **Создание директории конфигурации**
   - Создаёт /etc/dokploy
...
10. **Запуск основного сервиса Dokploy**
...
11. **Запуск Traefik (reverse proxy)**
...
Запуск / перезапуск Docker:

![dokploy3.png](dokploy3.png "Как мы видим проблема исправилась")
...
![dokploy4.png](dokploy4.png "Статус контейнеров Docker")
...
![dokploy5.png](dokploy5.png "Впервые при заходе в Dokploy вы устанавливаете вход: Setup the server")

![dokploy6.png](dokploy6.png "После setup'а вы попадаете в сам уже dokploy")

### Кастомизация

![dokploy7.png](dokploy7.png "Раздел кастомизации")
...
![dokploy8.png](dokploy8.png "Дополнительные настройки организации")

Можно изменить фавикон, имя организации и добавить новую организацию / сервер.

![dokploy9.png](dokploy9.png "Раздел Web Server")
...
![dokploy10.png](dokploy10.png "Раздел Profile")
...
![dokploy11.png](dokploy11.png "Раздел Git")
...
![dokploy14.png](dokploy14.png "Кнопка создания проекта")

![dokploy15.png](dokploy15.png "Форма создания проекта - пишите имя и описание (необязательно) и нажимаете Create")

![dokploy16.png](dokploy16.png "Результат создания проекта")

Далее в зависимости от необходимости вы можете настроить **Environments**

**Рассмотрим создание сервиса.**

![dokploy17.png](dokploy17.png "Выбор типа сервиса")

**Application** — ваши приложения (к примеру из github сайт)

![dokploy18.png](dokploy18.png "Application - обзор функций и варианты сборок видны на скриншоте")

**Templates** — готовые конфиги докер для быстрого развертывания сервисов.

![dokploy19.png](dokploy19.png "Выбор шаблонов докер")
...
![dokploy20.png](dokploy20.png "Ошибка с загрузкой шаблонов")

**Database** — создаются БД под ваши настройки.

![dokploy21.png](dokploy21.png "Выбор типа базы данных")
...
![dokploy22.png](dokploy22.png "Выбор Compose для создания кастомного стека")

Создаем композ

![dokploy23.png](dokploy23.png "Форма создания Compose")

После создания, заходим в созданный раздел и выбираем raw.

![dokploy24.png](dokploy24.png "Редактор raw YAML конфига")
...
![dokploy25.png](dokploy25.png "Выбор n8n шаблона")

Копируем конфиг и вставляем его в raw и нажимаем на save

![dokploy26.png](dokploy26.png "Сохранение YAML конфига")

Переходим в Environment Settings и добавляем необходимые переменные окружения:
...
![dokploy27.png](dokploy27.png "Настройка переменных окружения - возвращаемся в General и нажимаем Deploy")

![dokploy28.png](dokploy28.png "Логи n8n установки")

После запуска вас потом перекинет возможно в логи — там вы смотрите успешно ли все запустилось или нет.

После успешной установки можем проверить и зайти что n8n работает.

> Я менял немного под себя композ тк хотел поставить локально, если кому надо вот мой вариант:
...
![dokploy29.png](dokploy29.png "Сравнение Dokploy и Coolify")
