...
Чтобы прочувствовать тот "вайб" вайбкодера, нужно стать им. Но поскольку еще и параллельно нужно решать проблемы я эти [проблемы объединил](https://habr.com/ru/news/1013212/) и так получилась идея создания [HUB Hub](https://hub.hub.com), статическая платформа для документации с набором Markdown blocks, dev-панель, интерактивными таблицами, UI-компонентами и live-редактором.
...
Подмечу как пример: В ходе работы через pull request была обнаружена одна некритичная проблема, которая при ближайшем рассмотрении оказалась **false positive** / ([коммит](https://github.com/hub-projects/hub/commit/3fe7f983ce9e9c99696ecdc9379bfce1f8438d4a)). Это тоже часть процесса: ни один сканер(пока что) не даёт нулевого уровня ложных срабатываний, и умение их идентифицировать – такой же навык, как и исправление реальных уязвимостей.
