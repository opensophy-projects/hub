const e=`import React, { useRef, useEffect, useState } from 'react';
...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={\`w-full h-full flex items-center justify-center bg-black/10 text-gray-500 text-sm \${className}\`}
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
  ...
  return (
    <div
      ...
      style={{ position: 'absolute', inset: 0, mixBlendMode }}
    />
  );
};

export default LightPillar;`;export{e as default};
