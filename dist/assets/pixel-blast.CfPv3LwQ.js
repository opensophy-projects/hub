const e=`import { Effect, EffectComposer, EffectPass, RenderPass } from 'postprocessing';
...
function updateComposerTimeUniforms(composer: EffectComposer, time: number) {
  ...
  for (const p of composer.passes) {
    const pass = p as PassWithEffects;
...
function randomFloat(): number {
  if (typeof globalThis.window !== 'undefined' && globalThis.window.crypto?.getRandomValues) {
    ...
    globalThis.window.crypto.getRandomValues(u32);
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        destroyThree(threeRef.current, container);
        threeRef.current = null;
      }
      threeRef.current = buildThree({
        container,
        antialias,
        transparent,
        color,
...
type BuildThreeParams = {
  ...
  noiseAmount: number;
  autoPauseOffscreen: boolean;
  visibilityRef: React.MutableRefObject<{ visible: boolean }>;
...
function buildThree(p: BuildThreeParams): ThreeState {
  const {
    ...
    patternScale, patternDensity, pixelSizeJitter, enableRipples,
    rippleSpeed, rippleThickness, rippleIntensityScale, edgeFade,
    liquid, liquidStrength, liquidRadius, liquidWobbleSpeed, noiseAmount,
    autoPauseOffscreen, visibilityRef, speedRef
  } = p;
  ...
  const setSize = () => {
    ...
    if (p.threeRef.current?.composer) {
      p.threeRef.current.composer.setSize(renderer.domElement.width, renderer.domElement.height);
    }
  ...
  if (noiseAmount > 0) {
    ...
    if (composer.passes.length > 0) {
      type PassWithScreen = { renderToScreen?: boolean };
      for (const pass of composer.passes) {
        (pass as PassWithScreen).renderToScreen = false;
      }
    }
  ...
  const animate = () => {
    ...
    if (composer) {
      ...
      updateComposerTimeUniforms(composer, uniforms.uTime.value);
      composer.render();
    } else {
...
function updateUniforms(t: ThreeState, p: UpdateUniformsParams) {
  ...
  t.uniforms.uColor.value.set(p.color);
  t.uniforms.uScale.value = p.patternScale;
  t.uniforms.uDensity.value = p.patternDensity;
  t.uniforms.uPixelJitter.value = p.pixelSizeJitter;
  t.uniforms.uEnableRipples.value = p.enableRipples ? 1 : 0;
  t.uniforms.uRippleIntensity.value = p.rippleIntensityScale;
  t.uniforms.uRippleThickness.value = p.rippleThickness;
  t.uniforms.uRippleSpeed.value = p.rippleSpeed;
  t.uniforms.uEdgeFade.value = p.edgeFade;
  if (p.transparent) t.renderer.setClearAlpha(0);
  else t.renderer.setClearColor(0x000000, 1);
  if (t.liquidEffect) {
    updateLiquidUniforms(t.liquidEffect, p.liquidStrength, p.liquidWobbleSpeed);
  }
  if (t.touch) t.touch.radiusScale = p.liquidRadius;
}

export default PixelBlast;`;export{e as default};
